    ...
    while (true) {
    	...
    	if(pan.getPlayer() != null) {
    	  if( pan.crashed1 == 0 && pan.crashed2 == 0) {
		      ...
		      d1 = Math.sqrt((pan.pl1.x*20 - pan.getPlayer().x)*(pan.pl1.x*20 - pan.getPlayer().x) + (pan.pl1.y*20 - pan.getPlayer().y)*(pan.pl1.y*20 - pan.getPlayer().y));
		      d2 = Math.sqrt((pan.pl2.x*20 - pan.getPlayer().x)*(pan.pl2.x*20 - pan.getPlayer().x) + (pan.pl2.y*20 - pan.getPlayer().y)*(pan.pl2.y*20 - pan.getPlayer().y));
		      if(d1<90 || d2<110)
		      ...
		      if(d1<30) {
			      ...
			      if(d1 < 250) {
			    	force = 150/(d1*d1);
			    	angle = Math.atan2(Math.abs(pan.pl1.x*20-pan.getPlayer().x), Math.abs(pan.pl1.y*20-pan.getPlayer().y));
		      ...
		      if(d2<50 ) {
			      ...
			      if(d2 < 250) {
			    	force = 300/(d2*d2);
			    	angle = Math.atan2(Math.abs(pan.pl2.x*20-pan.getPlayer().x), Math.abs(pan.pl2.y*20-pan.getPlayer().y));
	      ...
	      if(pan.crashed1 > 0) {
	    	  pan.getPlayer().x = pan.pl1.x*20;
		      pan.getPlayer().y = pan.pl1.y*20;
	    	  pan.crashed1--;
	    	  if(pan.crashed1 == 0) {
	      ...
	      if(pan.crashed2 > 0) {
	    	  ...
	    	  pan.getPlayer().x = pan.pl2.x*20;
		      pan.getPlayer().y = pan.pl2.y*20;
	    	  pan.crashed2--;
	    	  if(pan.crashed2 == 0){
